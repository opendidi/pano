...
//素材管理页面，按类别获取素材
	...
	switch ($type) {
		...
		case 4:
			...
			break;
      case 5:
         $query_sql = "SELECT * FROM".$Base->table("cus_mediares")." WHERE pk_user_main = ".$user['pk_user_main']." AND media_type = 3 ORDER BY pk_media_res DESC LIMIT ".($page-1)*$pageSize.",".$pageSize;
         $count_sql = "SELECT COUNT(*) AS num FROM ".$Base->table("cus_mediares")." WHERE pk_user_main=".$user['pk_user_main']." AND media_type = 3";
         break;
   }
	$re;
	if ($query_sql != "") {
...
//上传全景素材
	...
	$result = KrOperation::slice($user['pk_user_main'],$imgs,$user['pk_user_main']."/works/");
	$imgsmain = $result['imgsmain'];
	//执行切图失败 
	if(empty($imgsmain)&&sizeof($imgsmain)<=0) {	
		$re['msg'] = '切图失败';
	}else{
		...
		foreach ($imgsmain as $value) {
			...
			$Db->insert($Base->table('imgsmain'),$value);
		}
		$re['status'] =1;
	}
	echo $Json->encode($re,JSON_NUMERIC_CHECK);
...
//上传视频
	...
	foreach ($params as &$v) {
		...
		$data = array(
				...
				'cover'=>KrOperation::get_video_thumb($v['location'],3)
			);
		// print_r($data);die;
   ...
   //贴片视频每次只能上传一个视频，上面的循环多余,多个太麻烦
   $obj = $params[0]['location'];//目标文件原地址

   $temp_dir = ROOT_PATH."data/mediares/".$user['pk_user_main']."/videotie/";//data/mediares后的路径和storage里的路径一样
   //storage的路径为244/video/1509454381845g72.mp4  对应本地路径为data/mediares/244/video/1509454381845g72.mp4
   Common::make_dir($temp_dir);
   $rpos = strrpos($obj,"/");
   $file = Transaction::get_storage($GLOBALS['_lang']['global_storage'])->downloadFile($obj,$temp_dir.substr($obj, $rpos==0 ? $rpos : $rpos+1));//视频下载
   if($file!=null){
      //文件下载完成
   }
